...
function droppoint(event) {
    ...
    if(option.split("_")[0] == "area"){
        option = event.target.id;
    }else{
        option = event.target.parentElement.id;
    }
    
    var kids = $('#' + option).children();
    
    var kid_0_id = kids[0].id;
    var kid_1_id = kids[1].id;
    var kid_2_id = kids[2].id;

    var kid_0 = kid_0_id.split("_")[1];
    var kid_1 = kid_1_id.split("_")[1];
    var kid_2 = kid_2_id.split("_")[1];

    var quesarray = data.split("_");
    var questionvalue = quesarray[1];

    if(kid_0 == questionvalue){
        setTimeout(function(){
            playAudio('right.wav');
        }, 300);
        $('#' + kid_0_id).append(document.getElementById(data));
        correct_counter++;
    }
    else if(kid_1 == questionvalue){
        setTimeout(function(){
            playAudio('right.wav');
        }, 300);
        $('#' + kid_1_id).append(document.getElementById(data));
        correct_counter++;
    }
    else if(kid_2 == questionvalue){
        setTimeout(function(){
            playAudio('right.wav');
        }, 300);
        $('#' + kid_2_id).append(document.getElementById(data));
        correct_counter++;
    }
    else{
        setTimeout(function(){
