...
function loadQuestion(itr) {
    ...
    for (i = 0; i < q_set[itr].length; i++) {
        ...
        if (total_que == current_que) {
            ...
            for (var key in opts) {
                k++;
                str += '<p style="font-size: 19px;" id="remove"><span class="strips" onclick=checkvalues(this);> <label id="q'+k+'" for="q' + (itr + 1) + '_' + k + '"> ' + key + ' </label><input type="radio" class="radio radio1 radio2" id="q' + (itr + 1) + '_' + k + '" name="q' + (itr + 1) + '" value="' + opts[key] + '" onclick=checkvalues(this); ></span></p>';
            }
            str += '</div>'
