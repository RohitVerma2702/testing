...
function loadQuestion(itr) {
    ...
    for (i = 0; i < q_set[itr].length; i++) {
        ...
        if (total_que == current_que) {
            ...
            for (var key in opts) {
                k++;
                str += '<p id="opt' + k + '" class="h6 position-absolute translate-middle"><input type="checkbox" class="invisible" id="q' + (itr + 1) + '_' + k + '" name="q' + (itr + 1) + '" value="' + opts[key] + '" onclick=checkvalues(this);><label for="q' + (itr + 1) + '_' + k + '">' + key + '</label></p>';
